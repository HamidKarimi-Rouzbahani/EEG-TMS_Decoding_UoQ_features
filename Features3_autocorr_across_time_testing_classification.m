...
for Subject=1:20
    Fs=1000;
    Dataset=1;
    subjs={'002','003','004','005','006','007','008','009','010','011','012','013','014','015','016','017','018','019','020','021'};
    load('D:\Selene\Hamid decoding\Animation data\order_subs_index.mat')
    load(fullfile ('D:\Selene\Hamid decoding\Animation data',['data sub',subjs{sortI(Subject)},'_allChannels_forDecoding.mat']));
    clearvars signal
    signal (:,:,1,:)= data_alltrials_cond1(1:64,:,:); % channel, time, cat, trial
    signal (:,:,2,:)= data_alltrials_cond2(1:64,:,:); % channel, time, cat, trial
    accuracy = nan*ones(35,nchoosek(size(signal,3),2),10,45);
    accuracy_perm = nan*ones(35,nchoosek(size(signal,3),2),10,45,10);
    ...
    for feature=[34 35]
        ...
        t=0;
        for times=1:numLags:410
            t=t+1;
            for channel = 1:size(signal,1)
                for category = 1:size(signal,3)
                    for trial = 1:size(signal,4)
                        %                     if feature==1
                        %                         trial_data=signal(channel,times:times+numLags*2,category,trial);
                        trial_data=signal(channel,times:times+numLags*2,category,trial)-nanmean(signal(channel,1:50,category,trial));
                        %                     else
                        ...
                        elseif feature==31 && channel==1 && category==1 && trial==1
                            feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),30); ccc
                        ...
                        elseif feature==34 && channel==1 && category==1 && trial==1
                            feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),21);
            ...
            Y=Y(2:end,:);
%             samples=[1:length(Y)];   
%             train_samples=randsample(length(Y),5);
            
%             Classifier_Model = fitcsvm(X,Y','Standardize',true,'KernelFunction','linear');
            Classifier_Model = fitcdiscr(X,Y','DiscrimType','Linear');

            CVSVMModel = crossval(Classifier_Model);
            classLoss = kfoldLoss(CVSVMModel);

%             [~,score] = predict(Classifier_Model,X);
%             [~,PredictedLabels]=max(score');
%             accuracy(feature,1,1,t)=sum(PredictedLabels==Y')./length(Y);
            accuracy(feature,1,1,t)=1-classLoss;
            clearvars -except accuracy_perm iteration numLags t Dataset Fs True_Predicted_labels accuracy Subject signal feature net
            [Subject feature t]
        end
        save(['Decoding_Accuracy_automatic_cross_validation',num2str(Subject),'.mat'],'accuracy','accuracy_perm');
    end
end
...
clear all;
for Subject=[1:20]
    load(['Decoding_Accuracy_automatic_cross_validation',num2str(Subject),'.mat']);
    accuracies(1,:,Subject)=squeeze(nanmean(accuracy(35,1,:,:),3));
    accuracies_perm(:,1:10,Subject)=squeeze(nanmean(accuracy_perm(35,1,:,:,:),3));
end
figure;
numLags=10;
A=errorbar([50:numLags:490]*2-100,nanmean(squeeze(accuracies(1,:,:))'),nanstd(squeeze(accuracies(1,:,:))'));
hold on;
B=errorbar([50:numLags:490]*2-100,nanmean(squeeze(nanmean(accuracies_perm,2))'),nanstd(squeeze(nanmean(accuracies_perm,2))'));
