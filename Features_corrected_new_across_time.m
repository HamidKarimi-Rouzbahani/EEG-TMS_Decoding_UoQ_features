        ...
        for condition=1:length(conditions)
            ...
            for trial=1:size(EEG.data,3)
                for channel=1:size(EEG.data,1)
                    signal(channel,1:length(time_window),condition,trial)=EEG.data(channel,801:2000,trial)-repmat(nanmean(EEG.data(channel,801:1000,trial)),[1 length(time_window)]); 
                end
            end
...
%               for feature=[2:9 11:13 18:27] % features when evaluating time windows
              for feature=[2:9 11 13 18:27] % features when evaluating time windows
                ...
                clearvars -except conditions Subjects time_window Windows Subjects bands lowband highband band windows windoww wind Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
        ...
        save(['Corrected3_Dec_DS_Claire_Att_Band_',Bands{band},'_Wind_sliding_Subject_',num2str(Subject),'.mat'],'accuracy');
    end
end
