...
% close all;
Dataset=1;
decoding=1; % 1=attention size; 2= coherence level
...
time_window=[1:1200]; % 1:1000 = pre-stimulus; 1000:2000 = post-stimulus
...
for band=bands   % 1=broad, 2=delta, 3=theta, 4=alpha, 5=beta, 6=gamma
    ...
    for Subject=1:length(Subjects)
        signal=nan*ones(61,length(time_window),2,500);
        %% reading the data
        for condition=1:length(main_conditions)
            clearvars EEG
            tmp_data=[];
            for the_other_condition=0:1
                if decoding==1
                    if the_other_condition==0
                        the_other_cond='cohLow';
                    elseif the_other_condition==1
                        the_other_cond='cohHigh';
                    end
                    EEG=pop_loadset('filename',['sub-',Subjects{Subject},'_GA_crit_',main_conditions{condition},'_',the_other_cond,'.set'],'filepath','C:\\Users\\mq20185770\\Documents\\MATLAB\\Claire\\Data\\');
                elseif decoding==2
                    if the_other_condition==0
                        the_other_cond='attendL';
                    elseif the_other_condition==1
                        the_other_cond='attendR';
                    end
                    EEG=pop_loadset('filename',['sub-',Subjects{Subject},'_GA_crit_',the_other_cond,'_',main_conditions{condition},'.set'],'filepath','C:\\Users\\mq20185770\\Documents\\MATLAB\\Claire\\Data\\');
                end
                tmp_data=cat(3,tmp_data,EEG.data);
            end
            for trial=1:size(tmp_data,3)
                for channel=1:size(EEG.data,1)
                    signal(channel,1:length(time_window),condition,trial)=tmp_data(channel,801:2000,trial)-repmat(nanmean(tmp_data(channel,801:1000,trial)),[1 length(time_window)]);
                end
            end
        end
        ...
        accuracy = nan*ones(35,1,length(Windows));
        for windoww=Windows
            ...
            %             for feature=[34] % features when evaluating time windows
            for feature=[2:9 11 13 18:30 32 34:35] % features when evaluating time windows
                ...
                for channel = 1:size(signal,1)
                    for category = 1:size(signal,3)
                        for trial = 1:size(signal,4)
                            ...
                            if feature==1
                                trial_data=double(signal(channel,1:200,category,trial));
                            else
                                trial_data=double(signal(channel,wind,category,trial));
                            end
                            ...
                            elseif feature==35 && Dataset<3 && channel==1 && category==1 && trial==1 % Autocorr
                                feature_chosen=nan.*ones(size(signal,1),size(signal,3),size(signal,4),window_span-1);
                            end
                ...
                clearvars -except main_conditions decoding window_span conditions Subjects time_window Windows Subjects bands lowband highband band windows windoww wind Dataset Fs True_Predicted_labels accuracy Subject signal feature X Y  net
        ...
        save(['New_Dec_DS_Claire_',main_conditions{1,1}(1:3),'_Wind_sliding_Subject_',num2str(Subject),'Cmplt_Feats.mat'],'accuracy');
    end
end
